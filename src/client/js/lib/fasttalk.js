...
exports.encode = (() => {
	var arrUint8 = new Uint8Array(1),
		arrUint16 = new Uint16Array(1),
		charUint16 = new Uint8Array(arrUint16.buffer),
		arrUint32 = new Uint32Array(1),
		charUint32 = new Uint8Array(arrUint32.buffer),
		arrFloat32 = new Float32Array(1),
		charFloat32 = new Uint8Array(arrFloat32.buffer),
		typeEncoder = (type, number) => {
			let output = '';
			switch (type) {
				case 'RawUint8':
					arrUint8[0] = number;
				...
				case 'String16':
					for (let i = 0, strLen = number.length; i < strLen; i++) output += typeEncoder('RawUint16', number.charCodeAt(i));
	...
	return (arr, verbose = false) => {
		...
		for (let i = 0; i < len; i++) integerView[i] = output.charCodeAt(i);
		if (verbose) {
...
exports.decode = (() => {
	var arrUint16 = new Uint16Array(1),
		...
		arrFloat32 = new Float32Array(1),
		charFloat32 = new Uint8Array(arrFloat32.buffer),
		typeDecoder = (str, type, offset) => {
			switch (type) {
				case 'Uint8':
					return str.charCodeAt(offset++);
				case 'Uint16':
					for (let i = 0; i < 2; i++) charUint16[i] = str.charCodeAt(offset++);
					return arrUint16[0];
				case 'Uint32':
					for (let i = 0; i < 4; i++) charUint32[i] = str.charCodeAt(offset++);
					return arrUint32[0];
				case 'Sint8':
				...
				case 'Float32':
					for (let i = 0; i < 4; i++) charFloat32[i] = str.charCodeAt(offset++);
					return arrFloat32[0];
				default:
	...
	return raw => {
		try {
			...
			let str = '';
			for (let i = 0, len = intView.length; i < len; i++) str += String.fromCharCode(intView[i]);
			let offset = 1;
					...
					case '8':
						{
							...
							for (let i = 0; i < len; i += 2) buf[i / 2] = typeDecoder(arr, 'Uint16', i);
