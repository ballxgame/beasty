...
class Shard extends EventEmitter {
    ...
    wsEvent(packet) {
        switch (packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
                ...
                { // (╯°□°）╯︵ ┻━┻
                    if (packet.d.guild_id === undefined) {
                        ...
                        if (packet.d.channel_id === null) {
                            ...
                            (channel.call || channel.lastCall)
                            .voiceStates.update(packet.d);
                        }
                        break;
                    }
                    var guild = this.client.guilds.get(packet.d.guild_id);
            ...
            case "TYPING_START":
                {
                    if (this.client.listeners("typingStart")
                        .length > 0) {
                        /**
            ...
            case "GUILD_ROLE_DELETE":
                {
                    ...
                    this.emit("guildRoleDelete", guild, guild.roles.remove({
                        id: packet.d.role_id
                    }));
                    break;
                }
            ...
            case "CALL_CREATE":
                {
                    ...
                    if (channel.call) {
                        ...
                        var incrementedID = "";
                        var overflow = true;
                        var chunks = packet.d.id.match(/\d{1,9}/g)
    ...
    checkReady() {
        if (!this.ready) {
            ...
            if (Object.keys(this.getAllUsersCount)
                .length === 0) {
                this.ready = true;
                /**
    ...
    onWSMessage(packet) {
        if (this.listeners("rawWS")
            .length > 0 || this.client.listeners("rawWS")
            .length) {
            /**
